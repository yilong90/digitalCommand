...
//
// MP3 Sound Decoder V5 DL1 Rev.exp.01
...
uint8_t gCV54_BEMFcoefficient = 72; 
...
//	mp3_set_serial (mySerial);	//set softwareSerial for DFPlayer-mini mp3 module 
	...
	//設定モード用
	pinMode(13, INPUT);// SCK pin sets input 
	digitalWrite(13, HIGH); //Internal pull up enabled
  ...
  gPreviousL5 = millis();
	...
	for( int i = 0; i < 27; i++)
	{
		gCVMP3Table[i] = Dcc.getCV( CV_MP3_F2 + i ) ;
...
void loop(){
    ...
    if(gSpeedCmd_vvvf != 0)
    {
      gIntervalRail =  255L * 900L / (unsigned long)gSpeedCmd_vvvf - 200;
      //gIntervalRail =  255L * 540L / (unsigned long)gSpeedCmd_vvvf - 200;
    }
		//Reset task
		gPreviousL2 = millis();
