...
unsigned long gPreviousL6 = 0;

void DoJoystickEvent(unsigned int inData);
...
void setup() {
	...
	Psx.setupPins(MASCON_dataPin, MASCON_cmndPin, MASCON_attPin, MASCON_clockPin, 10);
...
void loop()
{
	unsigned long aTime_Start = millis();
	...
	if( (millis() - gPreviousL5) >= 50)
	{
		...
		//Joystick
		unsigned int aPsxData = Psx.read();
		PutButtonBuffer(aPsxData);
		
		if( CheckButtonBuffer(aPsxData) == 1)
		{
			Joystick.GetHandleStatus(aPsxData);
			
			byte aButton = Joystick.GetButtonStatus(aPsxData);
			
			DoJoystickEvent(aButton);
			
			/*
			SerialDS.printHEX(aPsxData & MASK_BTN);
			SerialDS.print(", Handle: ");
			SerialDS.printDEC(Joystick.GetHandleValue());
			SerialDS.println("");
			*/
		...
		//Refresh load
		PutButtonBuffer(Psx.read());
		
		/*
		SerialDS.printHEX(aPsxData & MASK_BTN);
		SerialDS.print(", Speed: ");
		SerialDS.printDEC(Joystick.GetSpeedValue());
		SerialDS.print(", Handle: ");
		SerialDS.printDEC(Joystick.GetHandleValue());
		SerialDS.print(", Time: ");
		SerialDS.printDEC(millis() - aTime_Start);
	...
	if( (millis() - gPreviousL6) >= 250)
	{
		
		if( Joystick.Interval() > 0)
		{
			Sequence.RegisterSpeed(Joystick.GetSpeedValue());
		}
...
void DoJoystickEvent(unsigned int inData)
{
	...
	if( (inData & JOYSTICK_START) > 0)
	{
		//dir
		Sequence.RegisterDir();
		
	}
	if( (inData & JOYSTICK_SELECT) > 0)
	{
		if( (inData & JOYSTICK_A) > 0)
		{
			//F3
			Sequence.RegisterFunction(3);
		}
		if( (inData & JOYSTICK_B) > 0)
		{
			//F4
			Sequence.RegisterFunction(4);
		}
		if( (inData & JOYSTICK_C) > 0)
		{
			//F5
			Sequence.RegisterFunction(5);
		}
	...
	else
	{
		...
		{
			//F0
			Sequence.RegisterFunction(0);
		}
		if( (inData & JOYSTICK_B) > 0)
		{
			//F1
			Sequence.RegisterFunction(1);
		}
		if( (inData & JOYSTICK_C) > 0)
		{
			//F2
			Sequence.RegisterFunction(2);
		}
	}
