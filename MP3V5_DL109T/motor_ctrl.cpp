...
void MOTOR_Sensor()
{
	...
	int aSpeed_calculated = 0;       // BEMF enhance by MECY 2017/09/17
	
	//BEMF detection
	aBEMF_result = MOTOR_GetBEMF();
	
	aSpeed_calculated = aBEMF_result /6 ;    // BEMF enhance by MECY 2017/09/24
...
void MOTOR_Main(int inSpeedCmd, int inDirection)
{
	...
	if( gParam.mBEMFcutoff > 0)
	{
		...
		{
			aSpeedRef_offseted = MOTOR_PI(aSpeedRef - aSpeedDet, gParam.mPI_P, gParam.mPI_I, &gMotorPI_buf) + gParam.mStartVoltage ; //CV2 response  /MECY ReAdjust 2017/08/27
		}
	}
	else
	{
