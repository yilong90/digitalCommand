...
//
// MP3 Sound Decoder V5 DL1 Rev.exp.04
...
#define MAN_VER_NUMBER  8   /* Release Ver CV07 */
...
#define PIN_AMPUP	5	// アンプボリュームUP          //K1.2 Ajust by MECY 2017/02/12
#define PIN_AMPDOWN	6	// アンプボリュームDOWN        //K1.2 Ajust by MECY 2017/02/12
...
#define CV_SOUNDRailInt 51//fujigaya2                          //K1.2 Ajust by MECY 2017/02/12
#define CV_SOUNDLOOPTIMING 52                                 //Variable set of Sound loop time by MECY 2017/05/14
...
#define CV_AMP_Vol 59             //AMP Volume                //K1.2 Ajust by MECY 2017/02/12 
...
uint8_t gCV47_SoundSwitch = 10;//10：ディーゼル音
uint8_t gCV48_SoundState = 1;//0:レール継ぎ目音なし、1：レール継ぎ目音あり
uint8_t gCV49_SoundNotch = 1;//0:ノッチオフあり,1:ノッチオフなし
uint8_t gCV50_SoundVolume = 1;//ボリューム　0-4
uint8_t gCV51_SoundRailInt = 50;///レール継ぎ目音用最高速　0-100
uint8_t gCV52_SoundLoopTiming = 8; // Variable set of Sound loop time by MECY 2017/05/14
uint8_t gCV54_BEMFcoefficient = 64; // 0-127 step / Reajust by MECY 2017/04/16
uint8_t gCV55_PI_P = 18;
uint8_t gCV56_PI_I = 96;
uint8_t gCV57_BEMF_CutOff = 0;  // 0でBEMF Off 1でBEMF On

//MP3 Control
uint8_t gCV58_MP3_Vol = 28;
...
//アンプボリューム調整
uint8_t gAmpVol_Up;                                //K1.2 Ajust by MECY 2017/02/12
uint8_t gAmpVol_Down;                              //K1.2 Ajust by MECY 2017/02/12
...
//  {CV_29_CONFIG, 128},   //Make sure this is 0 or else it will be random based on what is in the eeprom which could caue headaches
  ...
  {CV_SOUNDVOLUME, 5},
  {CV_SOUNDRailInt,50},
  {CV_SOUNDLOOPTIMING, 8},
  {CV_BEMFcoefficient, 64},
  {CV_PI_P, 18},
  {CV_PI_I, 96},
  {CV_BEMFCUTOFF,0},
  {CV_MP3_Vol,24}, //5V PAM must be 28 or less
  {CV_AMP_Vol, 23},
...
void Play_VolDown(uint8_t inVolDown);                      //K1.2 Ajust by MECY 2017/02/12
void Play_VolUp(uint8_t inVolUp);                          //K1.2 Ajust by MECY 2017/02/12
...
void setup()
{
  ...
  gCV52_SoundLoopTiming = Dcc.getCV( CV_SOUNDLOOPTIMING );
	...
	VVVF_SetCV(52, gCV52_SoundLoopTiming);                  // Variable set of Sound loop time by MECY 2017/05/14
...
void loop()
{
	...
	{
		//VVVF control
		//VVVF_Cont(gSpeedCmd_vvvf);
//    VVVF_Cont(gSpeedCmd_vvvf, gFuncBits[3]); 
    VVVF_Cont(gSpeedCmd_vvvf, gFuncBits[3], gFuncBits[4]);               //F4 bit ctr Add by MECY 2017/10/07
    //Interval_Rail [ms]
...
///////MP3 ブレーキ音発生イベントルーチン Add by MECY 2017/04/19/////////
  if( (millis() - gPreviousL6) >= 100)
  {
    if((gCV48_SoundState & 2) != 0)
    {
...
///////////MP3 汽笛発生イベントルーチン Add by MECY 2017/04/19///////////
       if ( gSpeedCmd_vvvf <= 5) // 汽笛発生イベント検出
	  speed_state = 3;
       if ( gSpeedCmd_vvvf >= 6 && speed_state == 3){
	    speed_state = 4;
 	 mp3_set_volume (gCV58_MP3_Vol);
 	 delay (WAIT_VOLMP3);
 	 mp3_play (11);   //　汽笛 に対応するMP3 ファイルNOを指定する
	 }
      }
    //Reset task
    gPreviousL6 = millis();
...
//      Function F04
         ...
         {
         	PlayMP3_P(4);
                gFuncBits[4] = 1;  //F4 bit ctr Add by MECY 2017/10/07
         }    
         else
         {
         	PlayMP3_N(4);
                gFuncBits[4] = 0;  //F4 bit ctr Add by MECY 2017/10/07
         }
      break;
