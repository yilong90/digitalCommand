...
void VVVF_Cont(int inPWMFreq, uint8_t inF2Flag, uint8_t inF3Flag)
{
  uint16_t aPWMRef = 0;
  ...
  else
  {
    ...
    if ( gNotchSpdCount > 0)
    {
      if (inF3Flag == 1){
         ...
         if (nspeed < 2200)           //Not Shift down Sound in Notch Ctr Lock by  MECY 2017/10/07
         {
           gNotchSpdCount--;
         }
         else
         {
           // gNotchSpdCount--;        //Shift down Sound in Notch Ctr free by  MECY 2017/10/07
         }
      }
      else if(inF3Flag == 0)
      {
...
//    else if(nspeed > 500)
   else if((nspeed > 500) && (PrevSpeed >= 4))   //無負荷走行の時はエンジン音はLow by MECY 2017/06/25
    {
      gSound_playnext = 2;//accel
//      nspeed -= 30;
      nspeed -= gSoundLoopTiming * 2 ;  // Variable set of Sound loop time by MECY 2017/05/14
...
//    else
    else if( PrevSpeed < 4)  //停止している時だけidle音を鳴らす by MECY 2017/06/25
    {
  ...
  //サウンドオフ機構
  //SoundNotch == 2の時（手動操作時）にボリュームを操作
  if ((SoundNotch == 2)||(SoundNotch == 3))
  {
    if (inF2Flag == 0)
    {
      //ディーゼル音On
      
       SoundOn_count ++;                       //Sound On Deley  by  MECY 2017/10/07
      if((SoundOn_count > 80)&&(SoundNotch == 3))
      {
...
ISR(TIMER1_OVF_vect)
{
  ...
  if (pwm_shift != 8)
  {
    ...
    { //    リピート
  	...
  	uint8_t t = ima_decode(&imad, readSoundRom(gSound_played, sample), hilow);  // Sound extension by MECY 2017/08/02
